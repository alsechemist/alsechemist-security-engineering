...
.. code-block:: xml
    ...
    <decoder name="modsecurity_decoder">
        <program_name>modsecurity_audit_log</program_name>
        <prematch>^{"transaction":{</prematch>
        <plugin_decoder offset="after_prematch">JSON_Decoder</plugin_decoder>
    </decoder>

Each element of the decoder plays a specific role in identifying and parsing ModSecurity audit events.
...
.. image:: ../../assets/images/modsecurity-log-detection_wazuh/decoder-creation-1.png
...
You will also notice the ``decoder name`` has been changed to ``json``, it is because of the ``JSON_Decoder`` plugin that we used earlier. It hardcodes the name intentionally.
You can consider it as an overwriting feature of the plugin.
